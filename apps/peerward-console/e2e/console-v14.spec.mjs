...
test('three sharing kinds, real preview, permissions, pause and persistent issue state',async({page})=>{
 ...
 await d.getByRole('link',{name:'检查访问',exact:true}).click();await expect(page).toHaveURL(new RegExp(`policy\\?mesh=${mesh.id}.*resource=${resources[0].id}`));await expect(page.locator('.access-focus-banner')).toContainText(resources[0].name);await page.locator('#access-source').selectOption(`peer:${consumer.id}`);await expect(page).toHaveURL(/source=peer/);await expect(page.locator('.access-context-banner')).toContainText(consumer.display_name||consumer.name);await expect(page.getByRole('link',{name:'查看设备详情',exact:true})).toHaveAttribute('href',new RegExp(`resource=${consumer.id}`));await page.reload();await expect(page.locator('#access-source')).toHaveValue(`peer:${consumer.id}`);await expect(page.locator('.access-focus-banner').filter({hasText:resources[0].name})).toBeVisible();await expect(page.locator('.decision-chip.allowed')).toBeVisible();await screenshot(page,'access-results-zh');
 await page.setViewportSize({width:390,height:844});await expect(page.locator('.mobile-cell-label').filter({hasText:'访问结果'}).first()).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);await accessible(page);await screenshot(page,'mobile-access-results-zh');await page.setViewportSize({width:1440,height:1000});
 const row=page.getByRole('row').filter({hasText:resources[0].name});await row.getByRole('button',{name:'查看原因'}).click();await d.getByRole('button',{name:'撤销此授权'}).click();await d.locator('#grant-state-reason').fill('测试撤销');await d.getByRole('checkbox').check();await d.getByRole('button',{name:'确认提交',exact:true}).click();await expect.poll(async()=>(await(await simulate()).json()).allowed).toBe(false);
 await d.getByRole('button',{name:'Close / 关闭'}).click();await page.goto(`${origin}/operations?mesh=${mesh.id}`);await expect(page.getByText('修复后如何确认',{exact:true})).toBeVisible();const issuesPanel=page.locator('.issues-panel');await issuesPanel.getByRole('button',{name:'重新检查',exact:true}).click();await expect(issuesPanel.getByText(/重新检查完成/)).toBeVisible();const issue=page.locator('.issue-card').first();await expect(issue.locator('.issue-next-step')).toContainText('建议下一步');await expect(issue.locator('.issue-impact')).toBeVisible();await expect(issue.locator('.issue-primary-action')).toHaveAttribute('href',new RegExp(`mesh=${mesh.id}`));const acknowledgement=page.waitForResponse(response=>response.request().method()==='PATCH'&&new URL(response.url()).pathname.endsWith('/console/notices'));await issue.getByRole('button',{name:'标记已知'}).click();expect((await acknowledgement).ok()).toBe(true);await page.reload();await page.getByRole('button',{name:/^已知 \d+$/,exact:true}).click();await expect(page.locator('.issue-card')).not.toHaveCount(0);
});
...
test('LAN and exit grants revoke and restore from access details',async({page})=>{
 ...
 for(const kind of ['lan','internet']){
  ...
  await page.goto(`${origin}/policy?mesh=${mesh.id}`);await page.locator('#access-source').selectOption(`peer:${source.id}`);await page.locator('#access-resource-search').fill(name);await page.getByRole('row').filter({hasText:name}).getByRole('button',{name:'查看原因'}).click();const d=page.getByRole('dialog');
  ...
  await accessible(page);await d.getByRole('button',{name:'Close / 关闭'}).click();
 }
});
...
test('service simulation distinguishes configured transports and validates publisher address',async({page})=>{
 ...
 await page.goto(`${origin}/policy?mesh=${mesh.id}`);await page.locator('#access-source').selectOption(`peer:${source.id}`);await page.locator('#access-resource-search').fill(name);const row=page.getByRole('row').filter({hasText:name});await expect(row).toContainText('部分允许');await row.getByRole('button',{name:'查看原因'}).click();const d=page.getByRole('dialog');await d.getByText('高级：模拟具体条件',{exact:true}).click();
